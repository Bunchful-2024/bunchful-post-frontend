...
def general_prompt(platform, character_limit, topic, keyword):
    return f"""
Create a {platform} article highlighting the significance of {topic}. Generate the topic as clickbait-like and add a placeholder for {keyword}. 
Use primary keywords in italics, with a keyword density of less than 2% (i.e., less than twice per 100 words). 
Write in an engaging, conversational style using idioms and varied sentence structures, incorporating over 30% transition words for readability. 
Start with a compelling introduction related to {topic} and use clear headings or bullet points if needed. 
Include a link to a relevant resource or further reading and ensure the article length adheres to the recommended character count for optimal engagement: {platform}: {character_limit}. 
Add a clear CTA to encourage interaction, such as asking readers to comment, share, or follow a link. 
Pose a question at the end to prompt discussion. Use relevant images or media with descriptive alt text to enhance visual appeal, and strategically use hashtags such as #BeBunchful, #BunchfulSDGs, #GivingBack, #SocialImpact, #NewYork, #UNSDGs, #Philanthropy to increase the article’s reach and engagement. 
Mention and tag relevant organizations or individuals to increase visibility and interaction. 
Proofread the content to ensure it is free of grammatical errors and readability. 
Enhance the article with emojis and remove any placeholders. Use "Bunchful Enterprise" when mentioning company names.

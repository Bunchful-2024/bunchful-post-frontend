...
#Ignore This File, I have copy pasted everything in the prompts.py file, that would be our main file
...
def generate_linkedin_post(topic):
    linkedin_character_limit = 2000  # Example character limit
    prompt = general_prompt('LinkedIn', linkedin_character_limit)
    # Add code to generate the post using the prompt
    return prompt.format(Topic=topic)

def generate_facebook_post(topic):
    facebook_character_limit = 500  # Example character limit
    prompt = general_prompt('Facebook', facebook_character_limit)
    # Add code to generate the post using the prompt
    return prompt.format(Topic=topic)

def generate_instagram_post(topic):
    instagram_character_limit = 2200  # Example character limit
    prompt = general_prompt('Instagram', instagram_character_limit)
    # Add code to generate the post using the prompt
    return prompt.format(Topic=topic)

def generate_twitter_post(topic):
    twitter_character_limit = 280  # Example character limit
    prompt = general_prompt('X (Twitter)', twitter_character_limit)
    # Add code to generate the post using the prompt
    return prompt.format(Topic=topic)

def generate_threads_post(topic):
    threads_character_limit = 500  # Example character limit
    prompt = general_prompt('Instagram Threads', threads_character_limit)
    # Add code to generate the post using the prompt
    return prompt.format(Topic=topic)
